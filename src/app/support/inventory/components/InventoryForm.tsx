...
interface InventoryFormProps {
	initialData?: InventoryData & { id?: string };
	isEdit?: boolean;
}
	...
	const onSubmit = async (data: InventoryData) => {
		...
		try {
			if (isOffline) {
				...
				router.push("/support/inventory");
				return;
			}

			const url =
				isEdit && initialData?.id
					? `/api/support/inventory/${initialData.id}`
			...
			toast.success(`Inventory ${isEdit ? "updated" : "created"} successfully`);
			router.push("/support/inventory");
		} catch (error) {
